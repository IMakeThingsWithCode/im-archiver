...
curl_then_sift () {
  ...
  curl -s "https://accessim.org${1}" | grep -E "<a class=\"${2}\"${3}[^>]*>" -o
}
...
read -rp "↳ " selected_course # TODO enforce to options
...
echo -e "\n\n ⇁ Your files are in $(pwd)""$(echo "$lesson_url" | grep -E "\/accessim.org\/([^\/]*\/){2}" -o)"